...
class OpenAIServer:
    ...
    async def await_disconnected(self, raw_request: Request, promise):
        ...
        if not promise.finished:
            try:
                ...
                # Executor or IPC may already be torn down; ignore abort errors
                logger.warning(f"Abort on disconnect failed: {e}")
    ...
    async def health(self) -> Response:
        # Healthy only if executor exists, is not shutting down, and workers (if any) are alive
        executor = getattr(self.llm, "_executor", None)
        try:
            is_shutdown = executor.is_shutdown() if executor is not None else True
        except Exception:
            is_shutdown = True

        if executor is None or is_shutdown:
            return Response(status_code=503, content="executor not ready")

        # If available, verify worker pool health (PyTorch backend uses mpi4py workers)
        try:
            workers_started = getattr(executor, "workers_started", None)
            if workers_started is False:
                return Response(status_code=503, content="executor workers not started")

            mpi_futures = getattr(executor, "mpi_futures", None)
            if mpi_futures:
                for f in mpi_futures:
                    if f.done() and f.exception() is not None:
                        return Response(status_code=503, content="executor worker failed")
        except Exception:
            # Conservatively mark unhealthy if worker health probing fails
            return Response(status_code=503, content="executor worker health unknown")

        # Optional strict mode: perform a minimal generation probe when env requests it
        if os.getenv("TRTLLM_HEALTH_MODE", "").lower() in {"gen", "generate", "strict"}:
            gen_resp = await self.health_generate()
            if gen_resp.status_code != 200:
                return Response(status_code=503, content="generation probe failed")
    ...
    async def health_generate(self) -> Response:
        ...
        try:
            ...
            health_request = ChatCompletionRequest(
                ...
                stream=False,
                temperature=0.01 # Small positive temperature to satisfy validation
            )
    ...
    async def openai_chat(self, request: ChatCompletionRequest, raw_request: Request) -> Response:
        ...
        try:
            ...
            if request.prompt_token_ids is not None:
                ...
                # Fallback if model_config or chat template is unavailable
                if self.model_config is None or getattr(self.model_config, "model_type", None) is None:
                    prompt = conversation[-1]["content"] if conversation and conversation[-1].get("content") else "hi"
                else:
                    try:
                        ...
                        logger.warning("Chat template resolution failed; falling back to raw content")
                        prompt = " ".join([
