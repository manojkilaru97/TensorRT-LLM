...
class BaseLLM:
    ...
    def generate_async(
        ...
        # Log concise request details instead of dumping full prompt
        num_prompt_tokens = len(prompt_token_ids)
        num_query_tokens = len(query_token_ids) if query_token_ids is not None else 0
        logger.info(
            f"Generation request: prompt_tokens={num_prompt_tokens} query_tokens={num_query_tokens} max_new_tokens={sampling_params.max_tokens} stream={streaming}")
